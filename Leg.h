...
public:
    ...
    void attach(Adafruit_PWMServoDriver pwmBoard, int coxaPin, int femurPin, int tibiaPin)
    {   
        if (_debug)
            return;
    ...
    {
        // dest — это точка, куда надо тянуться
        // _cStart — координаты точки крепления (и центра вращения)
        // hDist - расстояние от _cStart до dest в горизонтальной плоскости
        // cAngle — это уже тот угол, на который нужно будет отклониться сервоприводу от его начального угла
        // DONT_MOVE — это просто флаг, означающий что coxa не нужно никуда вращать
        // _cStartAngle — это угол в пространстве, на который повернут серво по деволту
        ...
        // localDestX и localDestY — это координаты целевой точки
...
private:
    ...
    void move(float cAngle, float fAngle, float tAngle)
    {
        ...
        {
            if (cAngle != DONT_MOVE)
                _pwmBoard.setPWM(_coxaPin, 0, getPulseFromDeg(coxaLimit(mc)));
            if (fAngle != DONT_MOVE) 
                _pwmBoard.setPWM(_femurPin, 0, getPulseFromDeg(femurLimit(mf)));
            if (tAngle != DONT_MOVE)
                _pwmBoard.setPWM(_tibiaPin, 0, getPulseFromDeg(tibiaLimit(mt)));
        }
        else
        {
            log("Angles:");
...
private:
    ...
    Adafruit_PWMServoDriver _pwmBoard;
};
